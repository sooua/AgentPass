...
| **🎫 Scoped agent 令牌** | 两级按 agent 令牌:**标准**(reveal · checkout · list · rotate,覆盖全部目标)或 **Root**(增加 `admin`)。作用域在 handler 前强制,按身份归属。 |
...
<table>
<tr>
<td width="50%"><img src="docs/assets/agent-tokens.png" alt="Scoped agent 令牌" /><br/><sub><b>Scoped agent 令牌</b> —— 标准或 Root,在 handler 前强制。</sub></td>
<td width="50%"><img src="docs/assets/rotation.png" alt="轮换任务" /><br/><sub><b>轮换</b> —— reveal 后、N 次 reveal 后、或按周期。</sub></td>
</tr>
</table>
<sub>暖色明暗双主题 · 内置中英文。</sub>
</div>
...
**需要 [Node.js](https://nodejs.org) 22.5 及以上** —— 安装包内置 daemon 脚本,应用启动时用系统 Node 自动拉起。无需另开终端;若已有 daemon 在监听,应用直接接管,不会重复启动。
...
pnpm tauri:dev        # 原生桌面窗口(需 Rust);会自行拉起 daemon
```
...
稳定版(`v1.0.2`)。自动轮换默认关闭——在网关 provider 能把新密钥装到目标之前,计划内与 reveal 后的任务仅被创建、留待手动完成。`ssh_agent_socket` checkout 模式尚未实现;`temp_key_file` 是默认且唯一提供的模式。

## 品牌

<div align="center">
<img src="docs/assets/agentpass-brand-board.png" alt="AgentPass 品牌 —— 标记、字标、色板" width="720" />
</div>

钥匙孔标记三种适配背景的变体(terracotta / 浅色 / 深色)、横向锁版与暖色色板。源 SVG 与使用规范见 [`brand/logo/`](brand/logo/)。
